...
class VCPortfolioWithFunding:
    ...
    def create_comprehensive_portfolio(self):
        ...
        for vc in self.vc_list:
            ...
            # Find companies for this VC in integrated data
            vc_companies = [company for company in self.integrated_data if company['vc_name'] == vc_name]

            if not vc_companies:
                # If no companies found in integrated data, create a basic entry
    ...
    def save_to_json(self, filename='vc_portfolio_with_funding.json'):
    ...
    def save_to_csv(self, filename='vc_portfolio_with_funding.csv'):
