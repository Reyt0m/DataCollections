...
class VCPortfolioExtractor:
            ...
            for index, row in df.iterrows():
                ...
                if pd.notna(row.iloc[2]) and pd.notna(row.iloc[3]):  # URL and VC name columns
                    ...
                    vc_data.append({
                        ...
                        'vc_name': vc_name,
                        'vc_url': vc_url,
                        'location': row.iloc[10] if pd.notna(row.iloc[10]) else '',  # Location column
                        'year_founded': row.iloc[8] if pd.notna(row.iloc[8]) else '',  # Year Founded column
                        'affiliation_type': row.iloc[9] if pd.notna(row.iloc[9]) else '',  # Affiliation Type column
                        'example_investments': row.iloc[14] if pd.notna(row.iloc[14]) else '',  # Example of Investments
                        'example_exits': row.iloc[15] if pd.notna(row.iloc[15]) else ''  # Example of Exits
                    })
                    count += 1
    ...
    def is_likely_company_name(self, text):
        ...
        skip_words = [
            ...
            'podcast', 'history', 'recruit', 'website', 'spotify', 'see all',
            'articles', 'news', 'blog', 'contact', 'about', 'home', 'privacy',
            'terms', 'cookie', 'login', 'signup', 'search', 'menu', 'copyright',
            'all rights reserved', 'follow us', 'subscribe', 'team', 'company',
            'platform', 'information', 'mail magazine', 'value', 'jaen'
        ]
        ...
        if re.match(r'^[A-Za-z0-9\s&.,\-()]+$', text):
            ...
            if re.search(r'[A-Za-z]', text):
                ...
                if len(text.split()) <= 5:  # Company names are usually short
                    ...
                    if len(text.split()) == 1 and text.lower() in ['podcast', 'history', 'recruit', 'website', 'spotify', 'articles']:
                        return False
                    return True
...
def main():
    ...
    # Load VC data from CSV (limit to 20 for testing)
    vc_data = extractor.load_vc_data('Dissertation - VC list probided by startup db.csv', limit=20)
