...
def test_single_researcher():
    ...
    try:
        logger.info(f"研究者の研究課題を取得中: {researcher_url}")

        # 研究者の詳細情報を取得
        detailed_info = scraper.get_researcher_detailed_info(researcher_url)
        logger.info(f"研究者情報: {detailed_info.get('name', 'Unknown')}")

        # 研究課題を取得
        projects = scraper.get_research_projects_enhanced(researcher_url)
        logger.info(f"取得された研究課題数: {len(projects)}")
        ...
        output_file = "test_single_researcher_results.json"
        with open(output_file, 'w', encoding='utf-8') as f:
...
def test_basic_mode():
    ...
    try:
        logger.info(f"Basic modeで研究者の研究課題を取得中: {researcher_url}")

        # 研究課題を取得
        projects = scraper.get_research_projects_basic(researcher_url)
        logger.info(f"取得された研究課題数: {len(projects)}")
        ...
        output_file = "test_basic_mode_results.json"
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
...
def main():
    ...
    logger.info("Research Map統合スクレイパーのテストを開始")
    ...
    # 2. 単一研究者のテスト
    logger.info("\n=== 単一研究者のテスト ===")
    single_results = test_single_researcher()

    # 3. Basic modeのテスト
    logger.info("\n=== Basic modeのテスト ===")
    basic_results = test_basic_mode()

    # 結果の比較
    logger.info("\n=== 結果の比較 ===")

    if html_results:
        logger.info(f"HTML抽出: 総数{html_results['total_projects']}件, 競争的{html_results['competitive_projects']}件")

    if single_results:
        logger.info(f"単一研究者: 総数{single_results['total_projects']}件, 競争的{single_results['total_competitive_projects']}件")

    if basic_results:
        logger.info(f"Basic mode: 総数{basic_results['total_projects']}件, 競争的{basic_results['total_competitive_projects']}件")
