...
class GlobisPortfolioExtractor:
    ...
    def extract_portfolio_companies(self, html_content):
        ...
        # Based on the website content provided, the companies are listed in a specific format
        # Look for the pattern: "Company Name->Description"

        # Get all text content
        all_text = soup.get_text()

        # Look for company entries in the format shown in the website
        # Pattern: Company Name->Description
        company_entries = re.findall(r'([A-Z][a-zA-Z0-9\s&.,\-()]+)->([^->]+)', all_text)

        logger.info(f"Found {len(company_entries)} potential company entries")
        ...
        # If no companies found with the pattern, try alternative methods
        if not companies:
            logger.info("No companies found with pattern matching, trying alternative methods...")
            ...
            # Method 3: Look for elements containing specific keywords
            if not companies:
                # Look for elements containing investment stages or categories
                investment_keywords = ['Seed', 'Early', 'Growth', 'Consumer', 'Business', 'Frontier']

                for keyword in investment_keywords:
                    elements = soup.find_all(string=re.compile(keyword, re.IGNORECASE))
                    for element in elements:
                        if element.parent:
                            text_content = element.parent.get_text(strip=True)
