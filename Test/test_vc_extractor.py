...
class VCPortfolioExtractor:
    ...
    def load_vc_data(self, csv_file, limit=5):
        """Load VC data from CSV file with limit for testing"""
        try:
            ...
            for index, row in df.iterrows():
                ...
                if pd.notna(row.iloc[2]) and pd.notna(row.iloc[3]):  # URL and VC name columns
                    ...
                    vc_data.append({
                        ...
                        'vc_name': vc_name,
                        'vc_url': vc_url,
                        'location': row.iloc[8] if pd.notna(row.iloc[8]) else '',
                        'year_founded': row.iloc[7] if pd.notna(row.iloc[7]) else '',
                        'affiliation_type': row.iloc[8] if pd.notna(row.iloc[8]) else '',
                        'example_investments': row.iloc[13] if pd.notna(row.iloc[13]) else '',
                        'example_exits': row.iloc[14] if pd.notna(row.iloc[14]) else ''
                    })
                    count += 1

            logger.info(f"Extracted {len(vc_data)} VCs with valid URLs (limited to {limit} for testing)")
            return vc_data
    ...
    def save_results(self, output_file='test_vc_portfolio_database.json'):
    ...
    def create_excel_report(self, output_file='test_vc_portfolio_database.xlsx'):
...
def main():
    ...
    # Load VC data from CSV (limit to 5 for testing)
    vc_data = extractor.load_vc_data('Dissertation - VC list probided by startup db.csv', limit=5)
